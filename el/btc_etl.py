...
def get_max_btc_db_block(clickhouse_client, target="both"):
    if target == "both":
        blocks_max_block = clickhouse_client.execute(
            "select max(number) from bitcoin.blocks_raw"
        )[0][0]
        transactions_max_block = clickhouse_client.execute(
            "select max(block_number) from bitcoin.transactions_raw"
        )[0][0]
        assert (
            blocks_max_block == transactions_max_block
        ), "max block for blocks and transactions do not match"
        max_block = blocks_max_block
    elif target == "block":
        sql = "select max(number) from bitcoin.blocks_raw;"
        max_block = clickhouse_client.execute(
            "select max(number) from bitcoin.blocks_raw"
        )[0][0]
...
def get_new_btc_data(clickhouse_client, uri, start_block, end_block, target="both"):
    ...
    if target == "block":
        ...
        while try_counter <= 10 and data_pull_succeeded == False:
            try:
                ...
                print(e)
                print(
                    "failed getting block data from quicknode on try {try_counter}".format(
    ...
    # get new transactions
        ...
        while try_counter <= 10 and data_pull_succeeded == False:
            try:
                ...
                print(e)
                print(
                    "failed getting transaction data from quicknode on try {try_counter}".format(
                        try_counter=try_counter
                    )
                )
        if try_counter > 10:
        ...
        while try_counter <= 10 and data_pull_succeeded == False:
            try:
                ...
                print(e)
                print(
                    "failed getting enriched transaction data from quicknode on try {try_counter}".format(
                        try_counter=try_counter
                    )
                )
        if try_counter > 10:
    ...
    # get new blocks and transactions
        ...
        while try_counter <= 10 and data_pull_succeeded == False:
            try:
                blocks, transactions = btc_block_transaction_exporter(
                    uri, start_block, end_block
                )
                data_pull_succeeded = True
            except Exception as e:
                try_counter += 1
                print(e)
                print(
                    "failed getting block & transaction data from quicknode on try {try_counter}".format(
                        try_counter=try_counter
                    )
                )
        if try_counter > 10:
...
def txn_json_to_df(transactions_json, transaction_inputs, transaction_outputs):
    ...
    transactions_df["block_timestamp"] = pd.to_datetime(
        transactions_df.block_timestamp, unit="s", origin="unix"
    )
    transactions_df["block_timestamp"] = pd.DatetimeIndex(
        transactions_df.block_timestamp
    )
    transactions_df["is_coinbase"] = transactions_df.is_coinbase.map(
        {True: 1, False: 0}
    )
    ...
    if len(transaction_inputs) > 0:
        ...
        transaction_inputs_df = transaction_inputs_df.rename(
            columns={"index": "input_index"}
        )
        transaction_inputs_df["block_timestamp"] = pd.to_datetime(
            transaction_inputs_df.block_timestamp, unit="s", origin="unix"
        )
    else:
        transaction_inputs_df = pd.DataFrame(
            columns=[
                "input_index",
    ...
    transaction_outputs_df = transaction_outputs_df.rename(
        columns={"index": "output_index"}
    )
    transaction_outputs_df["block_timestamp"] = pd.to_datetime(
        transaction_outputs_df.block_timestamp, unit="s", origin="unix"
    )
...
def load_transactions(
    transactions_df, transaction_inputs_df, transaction_outputs_df, clickhouse_client
):
    txns_load_sql = """
...
def transform_load_btc_data(new_data, clickhouse_client):
    ...
    if "blocks" in new_data.keys() and "transactions" in new_data.keys():
        ...
        transactions_df, transaction_inputs_df, transaction_outputs_df = txn_json_to_df(
        ...
        load_transactions(
            transactions_df,
            transaction_inputs_df,
            transaction_outputs_df,
            clickhouse_client,
        )
...
def extract_transform_load_btc(
    clickhouse_client,
    node_uri,
    pg_db,
    target="both",
    ...
    logger.info(
        f"getting bitcoin data... ${job_row}", extra={"json_fields": log_details}
    )
    try:
        new_data = get_new_btc_data(
            clickhouse_client, node_uri, start_block, end_block, target
        )
    except Exception as e:
        log_details["error"] = e
        logger.info(
            f"failed getting bitcoin data... ${job_row}:",
    ...
    logger.info(
        f"completed getting bitcoin data... ${job_row}",
        extra={"json_fields": log_details},
    )

    ######transform and load btc data##########
    logger.info(
        f"transforming and loading new bitcoin data... ${job_row}",
        extra={"json_fields": log_details},
    )
    try:
        ...
        log_details["error"] = e
        logger.info(
            f"failed loading bitcoin data... ${job_row}:",
            extra={"json_fields": log_details},
        )
        updateJobRow = {
            "id": job_row["row"]["id"],
