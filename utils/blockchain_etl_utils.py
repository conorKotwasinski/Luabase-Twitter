...
class InMemoryObjectExporter:
    ...
    def _do_export(self):
        result = self.blockchain_streamer_adapter.export_all(self.start_block, self.end_block)
        return result
...
class EthInMemoryAdapter:
    def __init__(
            self,
            batch_web3_provider,
            item_exporter=InMemoryItemExporter(item_types=['block', 'transaction', 'log', 'token_transfer', 'trace', 'contract', 'token']),
            batch_size=100,
            max_workers=5,
            entity_types=','.join(['block', 'transaction', 'log', 'token_transfer', 'trace', 'contract', 'token'])):
